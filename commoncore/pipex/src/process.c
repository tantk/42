...
/*   Updated: 2023/11/04 19:59:14 by titan            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	aft_proc(t_proc *proc, int *counter)
{
	int	err;

	err = close(proc -> pipe_fd[1]);
	if (err == -1)
		ret_errmsg("exec_pipes, error with close pipe_fd[1]");
	if (*counter != 2)
	{
		err = close(proc -> prev_read_fd);
		if (err == -1)
			ret_errmsg("exec_pipes, error with close prev_read_fd");
	}
	proc -> prev_read_fd = proc -> pipe_fd[0];
	*counter += 1;
}

//continue pipes even if one of them has error
int	exec_pipes(t_proc *proc, int argc, char **argv)
{
	...
	while (counter <= (argc - 2))
	{
		...
		err = wait(&wstatus);
		if (exit_status(wstatus) == -1)
			ret_errmsg("fork_proc error with child process");
		aft_proc(proc, &counter);
	}
	return (1);
}
